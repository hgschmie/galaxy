...
module Galaxy::HostUtils
  class << self
    def logger ident="galaxy"
      @logger ||= begin
        ...
        class << log
          ...
          def debug * args
            args.length == 1 ? unsafe_debug(safe_format(args.first)) : unsafe_debug(* args)
          end

          alias_method :unsafe_info, :info

          def info * args
            args.length == 1 ? unsafe_info(safe_format(args.first)) : unsafe_info(* args)
          end

          def warn * args
            args.length == 1 ? warning(safe_format(args.first)) : warning(* args)
          end

          def error * args
            args.length == 1 ? err(safe_format(args.first)) : err(* args)
          end
  ...
  class CommandFailedError < Exception
    ...
    def message
      "Command '#{@command}' exited with status code #{@exitstatus} and output: #{@output.join("\n")}".chomp()
    end
  end
