...
module Galaxy
  ...
  class Daemon
    ...
    def self.pid_for pid_file
      begin
        File.open(pid_file) { |f| f.gets }.to_i
      rescue Errno::ENOENT
        return nil
      end
    end
    ...
    def self.kill_daemon pid_file
      ...
      raise "Cannot determine process id: Pid file #{pid_file} not found" if pid.nil?
    ...
    def go pid_file, log
      daemonize(log)
      File.open(pid_file, "w", 0644) { |f| f.write(Process.pid) }
      
      trap("TERM") { FileUtils.rm_f(pid_file); exit 0 }
      trap ("KILL") { FileUtils.rm_f(pid_file); exit 0 }
      @block.call
    end
